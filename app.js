...
function renderLinkScore(){
  ...
  const dims=[{key:'city',name:'城市等级'},{key:'gender',name:'性别'},{key:'age',name:'年龄档'},{key:'price',name:'价格档'},{key:'brand',name:'品牌'},{key:'model',name:'手机型号'}];
...
function processTab4() {
  ...
  readWorkbook(file, (wb, err) => {
    ...
    const processed = detail.map(r => {
      ...
      return { link, age:ageG, priceBand:pBand, brand, model, gender, city, period, valid, highAmt, hasHigh, highN, effN, allN, scoreN, totalOut };
    });
    // 维度定义
    const dimensions=[
...
function t4BuildExcel(d) {
  ...
  const dimOrder = ['city','gender','age','price','brand','model'];
  T4_LINK_ORDER.forEach(link => {
  ...
  let n=[
    ...
    ['3. 每个链路一个 sheet，包含6个画像维度（城市等级/性别/年龄档/价格档/品牌/手机型号）。'],
